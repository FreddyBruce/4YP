...
% Plots red labels for net senders and green fro net receivers

% Ioannis Psorakis, PARG 2010

function cg = community_graph_net(Groups,W)
%% INITIALIZATIONs
...
Rho = 0;
%% Calculate net sender vs reciever
[net, class, net_nodes, class_nodes] = net_emails_community(W, Groups);
%% PRODUCE GRID
...
title(strcat('Community network - Red = Sender, Green = Receiver'));
for i=1:N
    if class_nodes(i) == 1
        text(nodesXY(i,1),nodesXY(i,2),num2str(i),'BackgroundColor','red');
    else
        text(nodesXY(i,1),nodesXY(i,2),num2str(i),'BackgroundColor','green');
    end
end
hold off
end
