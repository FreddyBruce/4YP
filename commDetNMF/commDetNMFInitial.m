function [P,g,W,H,max_rank, invbeta] = commDetNMFInitial(V,max_rank,W0,H0,invbeta)
...
% invbeta: prior values of shrinkage parameters (optional)
% Outputs :
% 
...
% *** If there is one input argument - set the max rank as half the size of the
% first column (rounding up) ***
if (nargin < 2)
...
% *** This creates a probability distribution across the matrix ***
X = scale01(V);
...
% Initialise hyperprior
if (nargin < 5)
    invbeta = ones(max_rank,1);
end;
%now run the nmf with shrinkage priors
[W, H, invbeta] = nmfBayesMap(X, 1000,W0,H0, invbeta);
...
%--------------
function [W, H, invbeta] = nmfBayesMap(V,n_iter,W,H,invbeta)
...
%         invbeta - initial shrinkage priors
